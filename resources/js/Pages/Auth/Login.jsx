    ...
    return (
        <div style={S.page}>
            ...
            <div style={S.left}>
                ...
                <h1 className="w-title">Bon retour<br /><span className="w-accent">parmi nous</span></h1>
                <p className="w-desc">Connectez-vous pour accéder à votre espace de réservation.</p>
            ...
            <div style={S.right}>
                <div className="auth-card">
                    ...
                    <p className="auth-sub">Entrez vos identifiants</p>

                    {errors.email && (
                        <div className="err-box">⚠️ {errors.email}</div>
                    )}

                    <form onSubmit={submit}>
                        <div className="field">
                            <label className="lbl">Adresse Email</label>
                            <input className="inp" type="email" value={data.email}
                                onChange={e => setData('email', e.target.value)}
                                placeholder="exemple@institution.ma" autoFocus />
                        </div>

                        <div className="field">
                            <label className="lbl">Mot de passe</label>
                            <input className="inp" type="password" value={data.password}
                                onChange={e => setData('password', e.target.value)}
                                placeholder="••••••••" />
                        </div>

                        <button type="submit" className="btn-submit" disabled={processing}>
                            {processing ? <span className="spin" /> : 'Se connecter →'}
                        </button>
                    </form>

                    <p className="auth-footer">
                        Pas encore de compte ? <Link href="/register" className="auth-link">S'inscrire</Link>
                    </p>
                </div>
            </div>
        </div>
...
const S = {
    ...
    right:  { width:'clamp(340px,38vw,440px)', display:'flex', alignItems:'center', justifyContent:'center', padding:'32px', position:'relative', zIndex:2 },
};
...
.w-desc{font-size:16px;color:rgba(255,255,255,.58);line-height:1.75;max-width:380px;margin-bottom:36px}
.w-stats{display:flex;gap:32px}
.w-stat{display:flex;flex-direction:column}
.w-stat-n{font-family:'Syne',sans-serif;font-size:28px;font-weight:800;color:#fff}
.w-stat-l{font-size:12px;color:rgba(255,255,255,.45);margin-top:2px}
.auth-card{background:#fff;border-radius:24px;padding:36px 32px;width:100%;box-shadow:0 32px 80px rgba(0,0,0,.25);animation:popIn .6s cubic-bezier(.34,1.3,.64,1) both}
...
.err-box{background:#fee2e2;color:#dc2626;border-radius:10px;padding:10px 14px;font-size:13px;margin-bottom:16px;display:flex;align-items:center;gap:8px}
